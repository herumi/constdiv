...
そして，LLVM/GCC向けのパッチを作成し，後述のマイクロベンチマークにおいて，Intel Xeonで1.67倍，Apple M4（Apple MシリーズSoC）で1.98倍の高速化を達成した．
...
0以上の整数 $a$, 1以上の整数 $b$ について, $a$ を $b$ で割った商を $q$, 余りを $r$ とするとき $q:=\idiv{a}{b}=\floor{a/b}$, $r:=\imod{a}{b}$, $(q, r):=\divmod{a}{b}$ と表す．
すなわち $a = b q + r$ かつ $0 ≦ r < b$ である．
非負整数 $x$, $a$ に対して論理右シフトを $x>>a:=\idiv{x}{2^a}$ と定義する．
非負整数 $M$ を被除数のとりえる最大値, 除数を $d ∈ [1, M]$ とする．
条件 {\tt cond} が真のとき $x$, 偽のとき $y$ を返す関数を $\select({\tt cond}, x, y)$ と表す．
16/32/64/128ビット符号無し整数の型を {\tt u16/u32/u64/u128}, 符号あり整数の型を {\tt s16/s32/s64} などと表記する．
...
$d=2^a$（$a$は整数）の場合，$\idiv{x}{d} = x >> a$ と単純に論理右シフトに置き換える．
...
$y:=(x c_L)>>32$ とすると $(x c)>>32 = y+x$ となる．
...
ただし，数式通りに乗算命令を用いて32ビット×33ビット=65ビットを計算した後，論理右シフトをする場合x86-64では次の事情により効率が悪い．
x86-64における128ビット論理右シフト命令{\tt shrd}はSkylake-Xアーキテクチャで{\tt mul}と同じクロックサイクル・レイテンシが必要であるため\cite{anger}．
ベンチマークで使用したSapphire Rapidsアーキテクチャでも同様の傾向が見られた．

\tablecap{Skylake-Xアーキテクチャ: レイテンシとスループット}{
\label{TBL_xeon}
\begin{tabular}{|l|c|c|}
\hline
...
$c$ は33ビットで$d<\idiv{M}{2}$なので $a$ は33以上63以下となり $2^{64-a} c$ は64ビットに収まる．
また128ビット値は上位64ビット $H$，下位64ビット $L$ のレジスタ2個で表現されるため，64ビット論理右シフトは $H$ を取り出すだけとなり即値 $c$ の設定以外は乗算命令1回で済む．
Apple M4などのAArch64アーキテクチャでは，64ビット×64ビット→128ビット乗算命令は上位64ビットを返す{\tt umulh}と下位64ビットを返す{\tt mul}に分かれている．
...
Clangのコンパイラ基盤であるLLVM 23.0.0に提案方式の最適化を行う変更を行った\cite{LLVMopt}．
...
次にLLVM-IRから各CPU向けのアセンブリ言語を生成するためにLLVMのコマンドllcを用いた．
Xeonでは\texttt{llc -O2 -mattr=+bmi2 bench.ll -o <output>.s}，
M4では\texttt{llc -O2 -mtriple=arm64-apple-macosx -mcpu=apple-m4 bench.ll -o <output>.s}で生成した．
...
測定環境は，Xeon w9-3495X/Ubuntu 24.04.4 LTSとApple M4 MacBook Pro/Tahoe 26.4,
...
LLVMおよびGCCへの実装により，実機ベンチマークで最大1.67(x86-64), 1.98(Apple M4)倍の性能向上を確認した．
